...
此处记载了 BNUThesis 中所有值得留意的改动，格式参照 [Keep a Changelog](https://keepachangelog.com/en/1.0.0/)。
...
- 修改 committee.tex 文件，调整表格的列序，增加导师类型。


## [v0.0.31] - 2024-06-01
### Added
- 增加书脊的作者信息，使得书脊变为标题、作者、单位。

## [v0.0.3] - 2024-04-06

### Added
- 自动检测并使用 Windows 字体文件，默认从当前目录查找。通过添加字体可解决overleaf字体不一致的问题。

### Changed

- 修正 figure, table, algorithm 跨章节编号问题。
- 修正`\headheight is too small`的警告，修改致谢的署名格式（感谢冯振华提出的修改意见）。
- 修改`\eqref`的格式，改为英文括号。
