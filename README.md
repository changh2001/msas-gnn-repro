...
| 第6章 实验设置     | ogbn-arxiv 大图 Phase-W mini-batch 训练口径 | `training/alternating_opt.py`                                           |
...
- `outputs/tables/`：正文 LaTeX 表格（表 6.1–6.7，含近期效率补充表）
...
- 正文实验中所有依赖教师表示 `H*` 的分解式方法（`SDGNN-compatible`、`sdgnn_pure`、`MSAS-GNN`）统一采用三层 `GCN` 教师模型
- 正文消融表中同时区分两条 SDGNN 口径：`sdgnn_pure` 表示更贴近原论文的原始协议基线，`B0` 表示分层 BFS 候选池 + 全局 λ + 单次平坦 LARS/Lasso 的兼容基线
- `GCN` 教师默认配置为 `hidden_dim=64`、`lr=0.005`、`dropout=0.3`
- 正文敏感性分析默认扫描 `β_τ / τ_base`
- 新增 `graphsaint / nodeformer / difformer / sgformer / nagphormer` 为仓库内自包含复现实测口径
...
2. **SDGNN 口径**：`B0` 为兼容基线，它使用分层 BFS 候选池上的单个平坦 LARS/Lasso 子问题，并退化为全局统一 `lambda`；若需更贴近原论文的实现，请使用 `--ablation sdgnn_pure`，其采用 `K-hop + D-hop fanout` 平坦候选池。两者在当前默认实验中都统一采用 `GCN` 教师模型（见 `references/sdgnn_impl_notes.md`）
...
6. **大图训练口径**：ogbn-arxiv 按第6章实验设置仅在 Phase-W 使用 mini-batch，训练结束后会补做一次全图 Phase-Θ 重对齐
